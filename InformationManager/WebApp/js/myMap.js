...
MapManager.prototype.genMap = function(mapMeta, hotspotMeta) {
	...
	//because all vector data are generated by editMap.jsp
	//[original height, original width] are based on the [map_height, map_width] of editMap.jsp
	//this will ensure the scale of the vector layer and the offset between image layer and vector layer
	var original_height = $(window).innerHeight()-10;
	var original_width = original_height / $("#mapHeight").val() * $("#mapWidth").val();
...
var showMarker = function(evt) {
	...
	if (feature.geometry instanceof Geometry.Point) {
		...
		text = "户主：<a href='' style='margin-left: 5px;' target='_blank'>" + feature.data.hostName +"</a><br /><span style='position: absolute;'>备注：</span><div id='marker-description' style='margin-left: 45px;'>"
		...
		text = "<span style='top: 5px;position: absolute;'>备注：</span><div id='marker-description' style='margin-left: 45px;'>" 
			+ feature.data.description + "</div><div class='clear'></div>";
	}
